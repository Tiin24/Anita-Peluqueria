...
import data from "../../utils/Citas.json";
...
const TablaCitaId = () => {
  ...
  // useEffect(() => {
  //   if (clientId) {
  //     const clientData = data.filter((cita) => cita.clientId == clientId);
  //     if (clientData.length > 0) {
  //       setCitas(clientData);
  //     } else {
  //       setErrorMessage("No se encontraron citas para este cliente");
  //     }
  //   }
  // }, [clientId]);
  const { clientId } = useParams();
  ...
  return (
    <div className="flex flex-col items-center justify-center">
      <h2 className="font-poppins">
        Citas del Cliente{" "}
        <span className="text-2xl font-poppins"> {citas[0].client.name}</span>
      </h2>
      {errorMessage ? (
        <h1>{errorMessage}</h1>
      ) : citas.length === 0 ? (
        ...
        <table className="w-1/2 border-collapse border border-gray-300">
          ...
          <tbody>
            {citas.map((cita) => (
              <tr key={cita.id}>
                <td className="font-poppins border border-gray-300 px-4 py-2 text-center">
                  {cita.id}
                </td>
                <td className="font-poppins border border-gray-300 px-4 py-2 text-center">
                  {cita.client.name}
